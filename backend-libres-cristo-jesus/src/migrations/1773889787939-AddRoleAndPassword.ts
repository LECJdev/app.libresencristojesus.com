...
export class AddRoleAndPassword1773889787939 implements MigrationInterface {
  ...
  public async up(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `CREATE TYPE "public"."persona_rol_enum" AS ENUM('ADMIN', 'SUPER_ADMIN', 'INTEGRANTE')`,
    );
    await queryRunner.query(
      `ALTER TABLE "persona" ADD "rol" "public"."persona_rol_enum" NOT NULL DEFAULT 'INTEGRANTE'`,
    );
    await queryRunner.query(
      `ALTER TABLE "persona" ADD "password" character varying(255)`,
    );
  }
