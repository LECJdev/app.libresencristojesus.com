...
interface PublicAsistenciaCasaPaz {
  id: string;
  nombre: string;
  diaRegistro: string;
...
export default function RegistroCasaPazPublicoPage({
  ...
  const [asistencia, setAsistencia] = useState<PublicAsistenciaCasaPaz | null>(null);
  ...
  const fetchAsistenciaPublica = async () => {
    try {
      const { data } = await apiClient.get<PublicAsistenciaCasaPaz>(
        `/asistencias-casa-paz/public/${resolvedParams.token}`,
      );
      setAsistencia(data);
    } catch (error: unknown) {
  ...
  const registrarConDocumento = async (doc: string) => {
    ...
    try {
      const { data } = await apiClient.post<RegistroResponse>(
        `/asistencias-casa-paz/public/${resolvedParams.token}/registrar`,
        { documento: doc.trim() },
      );
  ...
  if (step === 'SUCCESS' || step === 'ALREADY_REGISTERED') {
    ...
    const message =
      step === 'ALREADY_REGISTERED'
        ...
        : 'Tu asistencia de casa de paz quedó registrada correctamente.';
    ...
    return (
      <div className="w-full max-w-md mx-auto mt-8 p-8 bg-white rounded-xl shadow-lg text-center">
        ...
        <div className="rounded-lg bg-slate-50 border border-slate-200 p-4 text-left text-sm text-slate-700 space-y-1">
          ...
          <p>
            <span className="font-semibold">Dirección:</span> {asistencia?.direccionCasa}
          </p>
          <p>
    ...
    return (
      <div className="w-full max-w-md mx-auto mt-8 p-6 bg-white rounded-xl shadow-lg">
        <h2 className="text-xl font-semibold text-slate-900 mb-1">
          Registro Casa de Paz
        </h2>
        <p className="text-sm text-slate-500 mb-2">{asistencia?.nombre}</p>
        <p className="text-sm text-slate-500 mb-5">
          Red: {asistencia?.red?.nombre || '—'} · {asistencia?.direccionCasa}
        </p>
