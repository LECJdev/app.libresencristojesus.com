...
export default function RegistroDominicalPublicoPage({
  ...
  const [asistencia, setAsistencia] = useState<PublicAsistencia | null>(null);
  ...
  if (step === 'SUCCESS' || step === 'ALREADY_REGISTERED') {
    ...
    const message =
      step === 'ALREADY_REGISTERED'
        ...
        : 'Tu asistencia dominical quedó registrada correctamente.';
    ...
    return (
      <div className="w-full max-w-md mx-auto mt-8 p-8 bg-white rounded-xl shadow-lg text-center">
        ...
        <div className="rounded-lg bg-slate-50 border border-slate-200 p-4 text-left text-sm text-slate-700 space-y-1">
          ...
          <p>
            <span className="font-semibold">Sede:</span> {asistencia?.sede?.nombre || '—'}
          </p>
          <p>
  ...
  if (step === 'ASK_DOCUMENT') {
    return (
      <div className="w-full max-w-md mx-auto mt-8 p-6 bg-white rounded-xl shadow-lg">
        <h2 className="text-xl font-semibold text-slate-900 mb-1">Registro Dominical</h2>
        <p className="text-sm text-slate-500 mb-5">
          {asistencia?.nombre} · {asistencia?.sede?.nombre || 'Sin sede'}
        </p>
